...
impl SuperElement<Self, TestCtx> for TestElement {
    ...
    fn with_downcast_val<'a, R>(
        ctx: &mut TestCtx,
        this: Self::Mut<'a>,
        f: impl FnOnce(&mut TestCtx, Mut<'_, Self>) -> R,
    ) -> (Self::Mut<'a>, R) {
        let ret = f(ctx, this);
        (this, ret)
    }
...
impl AnyElement<Self, TestCtx> for TestElement {
    fn replace_inner<'a>(_ctx: &mut TestCtx, this: Self::Mut<'a>, child: Self) -> Self::Mut<'a> {
...
impl ElementSplice<TestElement, TestCtx> for SeqTracker<'_> {
    fn with_scratch<R>(
        &mut self,
        ctx: &mut TestCtx,
        f: impl FnOnce(&mut TestCtx, &mut AppendVec<TestElement>) -> R,
    ) -> R {
    ...
    fn insert(&mut self, _ctx: &mut TestCtx, element: TestElement) {
        self.inner.active.push(element);
    }
    fn mutate<R>(
        &mut self,
    ...
    fn skip(&mut self, _ctx: &mut TestCtx, n: usize) {
        self.ix += n;
    }
    fn delete<R>(
        &mut self,
